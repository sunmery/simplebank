...
# Kubernetes集群地址
export CLUSTER_SERVER="https://116.213.43.175:6443"
#export CLUSTER_SERVER="https://kubernetes.default.svc"

# 仓库URL
export BRANCH="main"
# 仓库地址
export PROJECT_GIT_URL="https://github.com/sunmery/simplebank.git"

# Kubernetes 资源清单在仓库中的路径, 相对于仓库根目录的路径
export DEPLOY_PATH="deploy/frontend/kubernetes/argocd"

# 前端项目的命名空间, 不需要额外创建命名空间选择default即可
export NAMESPACE="bank1"

export PROJECT="bank1"

# argocd中的前端应用名, 用于分配团队人员的权限(查询/创建/同步/删除/操作)
export APPLICATION_NAME="bank-frontend1"
...
cat > deploy/frontend/kubernetes/argocd/frontend-proj.yml <<EOF
apiVersion: argoproj.io/v1alpha1
kind: AppProject
metadata:
  name: ${NAMESPACE} # 项目名称
  namespace: ${NAMESPACE} # 项目所在的命名空间，默认为argocd，可根据实际情况调整
spec:
  # 项目描述
...
#      kind: '*'
  ...
  - name: ${ROLE_NAME} # 角色名称
    ...
    - p, proj:${PROJECT}:${ROLE_NAME}, applications, get, ${APPLICATION_NAME}/*, allow
    - p, proj:${PROJECT}:${ROLE_NAME}, applications, create, ${APPLICATION_NAME}/*, allow
    - p, proj:${PROJECT}:${ROLE_NAME}, applications, sync, ${APPLICATION_NAME}/*, allow
    - p, proj:${PROJECT}:${ROLE_NAME}, applications, delete, ${APPLICATION_NAME}/*, allow
    # 允许查看集群信息
    - p, proj:${PROJECT}:${ROLE_NAME}, clusters, get, ${PROJECT_NAME}/*, allow
  orphanedResources:
    warn: true
...
# 创建前端项目应用
cat > deploy/frontend/kubernetes/argocd/frontend-app.yml <<EOF
apiVersion: argoproj.io/v1alpha1  # 指定 Argo CD API 版本
kind: Application  # 定义资源类型为 Application
metadata: # 元数据部分
  name: ${APPLICATION_NAME}   # 指定 Application 的名称
  namespace: ${NAMESPACE}  # 项目要部署到的命名空间
  # 定义资源的 finalizers
...
spec: # 规范部分
  project: ${PROJECT}  # 应用程序将被配置的项目名称，这是在 Argo CD 中应用程序的一种组织方式
  source: # 指定源
